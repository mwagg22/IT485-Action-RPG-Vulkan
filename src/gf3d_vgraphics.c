...
)
{
	...
	gf3d_mesh_init(4096);//TODO: pull this from a parameter
...
Bool gf3d_vgraphics_device_validate(VkPhysicalDevice device)
{
	...
	slog("Dedicated GPU: %i", (deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU) ? 1 : 0);
...
void gf3d_copy_camera(Matrix4 *dest){
	gfc_matrix_copy(dest,gf3d_vgraphics.ubo.view);
}
