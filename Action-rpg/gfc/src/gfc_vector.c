...
float vector3d_angle_between_vectors(Vector3D v1, Vector3D v2){
	float angle;
	Vector3D vec3;

	//get angle
	angle = acos(vector3d_dot_product(v1, v2) / ((sqrt(v1.x*v1.x + v1.y*v1.y + v1.z*v1.z))*(sqrt(v2.x*v2.x + v2.y*v2.y + v2.z*v2.z))));
	//cross

	vector3d_cross_product(&vec3, v1, v2);
	vector3d_normalize(&vec3);
	if (vec3.z == 1){
		angle=angle*-1;
	}
	if (vec3.z <1 && vec3.z >0){
		angle = angle*-1;
	}
	if ((isnan(angle))){
		angle = 0;
