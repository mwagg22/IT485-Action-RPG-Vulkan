...
void gf3d_obj_load_get_data_from_file(ObjData *obj, FILE* file)
{
      ...
      case 'v':
        switch(buf[1])
        {
          case '\0':
			...
			//slog("%f %f %f", x, y, z);
			if (obj->vertices[numvertices].x < minv.x){ minv.x = obj->vertices[numvertices].x; }
			if (obj->vertices[numvertices].y<minv.y){minv.y = obj->vertices[numvertices].y;}
			if (obj->vertices[numvertices].z<minv.z){ minv.z = obj->vertices[numvertices].z; }

			if (obj->vertices[numvertices].x>maxv.x){ maxv.x = obj->vertices[numvertices].x; }
			if (obj->vertices[numvertices].y>maxv.y){ maxv.y = obj->vertices[numvertices].y;}
			if (obj->vertices[numvertices].z>maxv.z){ maxv.z = obj->vertices[numvertices].z; }
            numvertices++;
			//slog("%i", numvertices);
  ...
  vector3d_copy(obj->minv,minv);
  //slog("min coor %f %f", obj->minv.x, minv.x);
  vector3d_copy(obj->maxv, maxv);
  //slog("max coor %f %f", obj->maxv.x, maxv.x);
}
